...
?>
	...
	<form class="cmxform" id="commentForm" action="save2db.php" method="post">
		<fieldset>
			...
			if ($_SESSION['found']==0)
			{
				echo "<br><br><center>Sorry, you are not allowed to approve task closing because your session is expired! Please re-login to <a href='http://portal.telkom.co.id'>POINT</a></center><br><br>";
			}
			else
			{
			*/
			...
			?>
			<p>
				<label for="cfileatt">File Attachment:</label>
				<textarea id="cfileatt" name="fileatt" rows="5" cols="97" readonly class="readonly"><?= $showfiles; ?></textarea>
			</p>
			<!--
			<p>
				<label for="cdwstatus">Download Status:</label>
				<input type="text" id="cdwstatus" name="dwstatus" size="50" value="<? if ($rows->download_status==1) echo "Invitation Only"; elseif ($rows->download_status==2) echo "Public"; else echo ""; ?>" readonly class="readonly"/>
			</p>
			-->
			<p>

...
switch ($_REQUEST['sw']) {
	...
	case "11" :
		...
		echo "$nik, $name, $email<br>";

		// send notification by e-mail
		include("sendmail_test.php");
	...
	case "14" :
		...
		$q="UPDATE sharing_activity SET id_confirm='$_REQUEST[id_confirm]' WHERE id_know='$_REQUEST[idk]' AND nik='$_SESSION[nik_login]'";
		#echo "<br>$q<br>-----";
		query_sql($q,$res);
	...
	case "16" :
		...
		$q="INSERT INTO sharing_activity (id_know,nik,id_confirm,id_inv_status) VALUES ('$_REQUEST[idk]','$_REQUEST[nik]','5','4')";
		echo "<br>$q<br>-----";
		#query_sql($q,$res);
		...
		// create new notification request sharing => send notification to commitee
		$q="INSERT INTO notification (id_not,id_know,created,subject) VALUES ('$id_not','$_REQUEST[idk]',NOW(),'$nm_subject')";
		echo "<br>$q<br>";
		#query_sql($q,$res);

		// create new notification for nik
		$q="INSERT INTO notification_4nik (id_not,nik) VALUES ('$id_not','$_REQUEST[nik]')";
		echo "<br>$q<br>";
		#query_sql($q,$res);

		// send email to committee
		// get requester
		$q="SELECT a.nama,a.email,b.id_bidang,b.nm_bidang FROM user a JOIN bidang b ON a.id_bidang=b.id_bidang WHERE nik ='$_REQUEST[nik]'";
		echo "$q<br>";
		$result = mysql_query($q);
		$rows = mysql_fetch_object ($result);
		$nik  = $_REQUEST[nik];
		...
		$q="SELECT a.judul,b.nama FROM knowledge a JOIN user b ON a.nik=b.nik WHERE id_know ='$_REQUEST[idk]'"; 
		echo "$q<br>";
		$result = mysql_query($q);
		$rows = mysql_fetch_object ($result);
		$id_know = $rows->id_know;
	...
	case "18" :
		...
		$q="UPDATE knowledge SET abstraksi='$_REQUEST[abstraksi]', harapan='$_REQUEST[harapan]', referensi='$_REQUEST[referensi]', sharing_status='5'".$upd_rdkey.", download_status='$dw_status' WHERE id_know='$_REQUEST[idk]'";
		#echo "<br>$q<br>";
		query_sql($q,$res);
	...
	case "19" :
		...
		$q="SELECT a.id_know,a.nik,a.judul,b.nama,b.email,b.id_bidang FROM knowledge a JOIN user b ON a.nik=b.nik WHERE id_know ='$_REQUEST[idk]'"; 
		#echo "$q<br>";
		$result  = mysql_query($q);
		$rows    = mysql_fetch_object ($result);
		$id_know = $rows->id_know;
...
// ---------- //
	...
	case "21" :
		...
		if ($_REQUEST[approval]==0) {
			$q="UPDATE knowledge SET t_mulai='$t_mulai', t_akhir='$t_akhir', lokasi='$_REQUEST[lokasi]', sharing_status='2', app_req_by='$_SESSION[nik_login]', app_req_at=NOW() WHERE id_know='$_REQUEST[idk]'";
			echo "$q<br>-----";
			#query_sql($q,$res);

			if (strlen($_REQUEST[catatan])>0) {
				$q = "INSERT INTO sharing_notes (id_know, nik, created, notes) VALUES ('$_REQUEST[idk]', '$_SESSION[nik_login]', NOW(), '$_REQUEST[catatan]')";
				echo "$q<br>-----";
				#query_sql($q,$res);
			}
		} else {

			$q="UPDATE knowledge SET t_mulai='$t_mulai', t_akhir='$t_akhir', lokasi='$_REQUEST[lokasi]', sharing_status='3', app_req_by='$_SESSION[nik_login]', app_req_at=NOW() WHERE id_know='$_REQUEST[idk]'";
			echo "1. Approval Request Sharing:<br>$q<br>-----";
			#query_sql($q,$res);
			...
			// create sharing activity table for speaker
			$tSQL="INSERT INTO sharing_activity (id_know,nik,id_confirm,id_inv_status,attend) VALUES ('$_REQUEST[idk]','$_REQUEST[niknya]','1','1','1')";
			echo "<br>2. Create sharing activity table for speaker:<br>$tSQL<br>-----";
			#query_sql($tSQL,$res);

			// create sharing activity table for member of speaker
			if (strlen($_REQUEST[member]) > 0) {
				$niks = explode(",",$_REQUEST[member]);
				#echo "<br>3. Create sharing activity table for member of speaker:";
				#echo "<br>\$_REQUEST[member]: $_REQUEST[member]<br>";
				$member = array();
				foreach ($niks as $key => $value) {
					$tSQL="INSERT INTO sharing_activity (id_know,nik,id_confirm,id_inv_status,attend) VALUES ('$_REQUEST[idk]','$value','1','2','1');";
					echo "<br>3. Create sharing activity table for member of speaker:<br>$tSQL<br>-----";
					#query_sql($tSQL,$res);
					$member[] = $value;
				}
				#echo "<br>-----";
			...
			if (count($bid) == $jml)
			{
				...
				$q="INSERT INTO sharing_activity (id_know,nik,id_inv_status) VALUES ('$_REQUEST[idk]',(SELECT nik FROM user WHERE ID_LOKER='100'),'3')";
				echo "<br>4. Create sharing activity table for SM RDC<br>$tSQL<br>-----";
				#query_sql($q,$res);
			}
			...
			foreach ($bid as $key => $value) {
				...
				$q="SELECT * FROM user WHERE id_loker LIKE '".substr($value,0,2)."%' AND active='1' AND nik <> '$_REQUEST[niknya]'".$qadd;
				echo "<br>5. Create sharing activity table for Audience<br>$q<br>-----";
				query_sql($q,$res);
				$result = mysql_query($q);
				while ($row = mysql_fetch_array ($result)) {
					$tSQL="INSERT INTO sharing_activity (id_know,nik,id_inv_status) VALUES ('$_REQUEST[idk]','$row[nik]','3');";
					echo "<br>$tSQL";
					#query_sql($tSQL,$res);
				}
				echo "<br>-----";
			}
			// create sharing activity table for External Audience
			if (strlen($_REQUEST[exaudien])>0) {
				$external = str_replace(" ","",$_REQUEST[exaudien]);
				$external = explode(",",$external);
				echo "<br>6. Create sharing activity table for External Audience:";
				#echo "<br>\$_REQUEST[exaudien]: $_REQUEST[exaudien]<br>";
				foreach ($external as $key => $value) {
					$tSQL="INSERT INTO sharing_act_ext (id_know,email) VALUES ('$_REQUEST[idk]','$value');";
					echo "<br>$tSQL";
					#query_sql($tSQL,$res);
				}
				#echo "<br>-----";
			...
			$q="SELECT a.id_know,a.nik,a.judul,a.lokasi,b.nama,b.email,b.id_bidang FROM knowledge a JOIN user b ON a.nik=b.nik WHERE id_know='$_REQUEST[idk]'"; 
			#echo "<br>$q<br>";
			$result = mysql_query($q);
			$rows   = mysql_fetch_object($result);
			$nik    = $rows->nik;
		...
		Header("Location: management.php?mn=1");
		break;
	...
	case "22" :
		...
		if ($_REQUEST[approval]==0)
		{	// report is rejected
			$q="UPDATE knowledge SET sharing_status='3', app_report_by='$_SESSION[nik_login]', app_report_at=NOW() WHERE id_know='$_REQUEST[idk]'";
			#echo "1. Approval Close Sharing Report:<br>$q<br>-----";
			query_sql($q,$res);
			if (strlen($_REQUEST[catatan])>0)
			{
				$q="INSERT INTO closing_notes (id_know,created,nik,notes) VALUES ('$_REQUEST[idk]',NOW(),'$_SESSION[nik_login]','$_REQUEST[catatan]')";
				#echo "$q<br>";
				query_sql($q,$res);
			}
		}
		...
		{	// report is approved!
			$q="UPDATE knowledge SET sharing_status='6', app_report_by='$_SESSION[nik_login]', app_report_at=NOW(), report_notes='$_REQUEST[catatan]' WHERE id_know='$_REQUEST[idk]'";
			#echo "1. Approval Close Sharing Report:<br>$q<br>-----<br>";
			query_sql($q,$res);

			// check if member of speaker, bidang & external audience is exist
			$q="SELECT member,inv_bidang,ext_audience FROM knowledge WHERE id_know='$_REQUEST[idk]'";
			#echo "2. Check if member of speaker, bidang & external audience is exist:<br>$q<br>-----<br>";
			query_sql($q,$res);
			$result = mysql_query($q);
			...
			if ($jmlbidang==6)
			{ $point = 20; $tipe=3; }
			elseif ($jmlbidang>1 && $jmlbidang<6)
			{ $point = 15; $tipe=2; }
			else
			{ $point = 10; $tipe=1; }
			...
			#echo "3. Check if member of speaker is exist:<br>$q<br>-----<br>";
			query_sql($q,$res);
			$result = mysql_query($q);
			$r = mysql_fetch_object ($result);
			#echo "\$r->hasil: $r->hasil<br>";
			if ($r->hasil > 1)
			{
			...
			#echo "Anda ber-$r->hasil, Nilai masing2: $yourpoint<br>";
			// compute submitter point based on daftar hadir
			$q="UPDATE sharing_activity SET poin='$yourpoint', tipe='$tipe' WHERE id_know='$_REQUEST[idk]' AND id_inv_status<'3'";
			#echo "4. Compute submitter point based on daftar hadir:<br>$q<br>-----<br>";
			query_sql($q,$res);

			// compute attendant point if exist based on daftar hadir
			$q="UPDATE sharing_activity SET poin=2, tipe=5 WHERE id_know='$_REQUEST[idk]' AND id_inv_status>2 AND attend=1 AND nik<>'602217'";
			#echo "5. Compute attendant point if exist based on daftar hadir:<br>$q<br>-----<br>";
			query_sql($q,$res);
		}

		Header("Location: management.php?mn=3");
		break;
	...
	case "23" :
		...
		// call from: sharing_detail.php
		// Status: Running
		$q="UPDATE req_to_attend SET approve_by='$_SESSION[nik_login]', approve_date=NOW() WHERE id_know='$_REQUEST[idk]' AND nik='$_REQUEST[nik]'";
		#echo "<br>$q<br>";
		query_sql($q,$res);

		$q="UPDATE sharing_activity SET id_confirm='$_REQUEST[id_confirm]' WHERE id_know='$_REQUEST[idk]' AND nik='$_REQUEST[nik]'";
		#echo "<br>$q<br>";
		query_sql($q,$res);

		// get subject & message from subject table
		$q="SELECT * FROM subject WHERE id_subject='4'";
		$result = mysql_query($q);
		$rows = mysql_fetch_object ($result);
		$nm_subject = $rows->nm_subject;
		$message = $rows->message;

		// create random id for id notification
		$id_not = md5(uniqid(rand(), true));

		// create approval request to attend notification
		$q="INSERT INTO notification (id_not,id_know,created,subject) VALUES ('$id_not','$_REQUEST[idk]',NOW(),'$nm_subject')";
		#echo "<br>$q<br>";
		query_sql($q,$res);

		// create request to attend notification for nik requester
		$q="INSERT INTO notification_4nik (id_not,nik) VALUES ('$id_not','$_REQUEST[nik]')";
		#echo "<br>$q<br>";
		query_sql($q,$res);

		// send email to nik requester
	...
	case "44" :
		...
		// call from: .php
		// Status: Running
		$q="INSERT INTO knowledge_map (id_map,nm_map,expert,level,id_top) VALUES ('$_REQUEST[id_map]', '$_REQUEST[nm_map]', '$_REQUEST[expert]', '$_REQUEST[level]', '$_REQUEST[id_top]')";
		echo "<br>$q<br>";
		#query_sql($q,$res);
	...
	case "51" :
		...
		$your_nik  = explode("-",$_REQUEST['nik']);
		// save to knowledge table
		$q="INSERT INTO knowledge (nik, member, id_map, jenis, judul, t_mulai, t_akhir, lokasi, unitkerja, inv_bidang, ext_audience, abstraksi, harapan, referensi, created, conflict) VALUES (, '$your_nik[0]', '$member', '$_REQUEST[id_map]', '$_REQUEST[jenis]', '$_REQUEST[judul]', '$t_mulai', '$t_akhir', '$_REQUEST[lokasi]', '$_REQUEST[unitkerja]', '$inv_bidang',  '$_REQUEST[exaudien]', '$_REQUEST[abstraksi]', NOW(), '$conflict')";
		#echo "$q<br>";
		query_sql($q,$res);

		// get knowledge that just inserted
		$q="SELECT a.id_know,a.nik,b.nama,b.email FROM knowledge a JOIN user b ON a.nik=b.nik ORDER BY id_know DESC LIMIT 1"; 
		#echo "<br>==>&nbsp;$q<br>";
		$result = mysql_query($q);
		$rows = mysql_fetch_object ($result);
		$id_know = $rows->id_know;
		...
		// get subject & message from subject table
		$q="SELECT * FROM subject WHERE id_subject='1'";
		$result = mysql_query($q);
		$rows = mysql_fetch_object ($result);
		$nm_subject = $rows->nm_subject;
		$message = $rows->message;
		$message .= $nama.".\r\n";
		$message .= "Please click <a href='http://localhost/km2/management.php?id=$id_know&mn=12'>here</a> to view ".$name."'s request.";

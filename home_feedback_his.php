...
// Step 2
$query  = "SELECT a.*, b.judul, c.nama ";
$query .= "FROM feedback a JOIN knowledge b ON a.id_know=b.id_know JOIN user c ON b.nik=c.nik ";
$query .= "WHERE a.nik='$_SESSION[nik_login]'";
...
<?
echo "</td><td align='right'><a href='?mn=5'>Feedback</a></td></tr></table>";
echo "<center>";

# print table header
echo "<table id='myTable' class='tablesorter' border='0' cellpadding='0' cellspacing='1'>";
echo "<thead><tr><th>No.</th><th>Title/Theme</th><th>Contributor</th><th>Created</th></tr></thead>";
echo "<tbody>";
...
?>
	<tr valign="top">
		...
		<td><a href="archives_detail.php?id=<?= $row["id_know"]; ?>&mn=1" title="View Sharing Knowledge Detail" class="thickbox"><?= $row["judul"]; ?></a></td>
		<td><?= $row["nama"]; ?></td>
...
{		
	echo "<tr><td colspan='5' align='center'>Sorry, no sharing knowledge feedback history at this time!</td></tr>";
	echo "</tbody>";
	echo "</table><p>";
}
echo "</center>";

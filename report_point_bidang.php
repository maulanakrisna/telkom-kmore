...
*/
$query  = "SELECT c.id_bidang, c.nm_bidang, SUM(a.poin) AS nilai FROM sharing_activity a ";
$query .= "JOIN bidang c ON a.id_bidang=c.id_bidang ";
$query .= "WHERE c.id_bidang<>100 AND a.id_inv_status < 3 AND poin IS NOT NULL AND a.id_know IN (";
$query .= "SELECT id_know FROM knowledge WHERE DATE(t_mulai) BETWEEN DATE('$tmulai') AND DATE('$takhir') ORDER BY t_mulai";
$query .= ") GROUP BY c.nm_bidang ORDER BY nilai DESC";
...
?>
	<tr valign="top">
		...
		<td><a href="report_point_bidang_d.php?idb=<?= $row["id_bidang"]; ?>&height=400&width=780" title="Sharing Point Bidang Detail" class="thickbox"><?= $row["nm_bidang"]; ?></a></td>

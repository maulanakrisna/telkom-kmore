...
*/
	...
	$sQuery = "SELECT a.*, b.nama AS submitter, c.nama AS speaker FROM knowledge a JOIN user b ON a.nik=b.nik JOIN user c ON a.nik=c.nik WHERE a.id_know='$_REQUEST[idk]'";
	#echo "$sQuery<br>";
	query_sql($sQuery,$result);
	$rows = mysql_fetch_object ($result);
	if ($rows->sharing_status=="1" && $rows->t_mulai<$tnow)
		$echostatus = " - <font color='red'>Expired</font>";
?>
	<br>
	<form name="myform" class="cmxform" id="commentForm" action="save2db.php" method="post">
		<fieldset>
			<h2>Edit Request Sharing<?= $echostatus; ?></h2>
			...
			<p>
				...
				<textarea id="cexaudien" name="exaudien" rows="5" cols="97"><?= $rows->ext_audience; ?></textarea><br>
			...
			<p>
				<label for="ccatatan">Show notes:</label>
				<a href="show_req_notes.php?id=<?= $_REQUEST[id]; ?>&height=300&width=600" title="Request Sharing Notes" class="thickbox"><img src="images/notes.gif" border="0" hspace="10" vspace="10"></a>
			</p>
			<p>
				<label>&nbsp;</label><input type="submit" name="btnSubmit" class="button" value="Submit"/>&nbsp;
				<? if ($rows->sharing_status=="1" && $rows->t_mulai<$tnow) { ?>
				<input type="submit" name="btnSubmit2" value="Close Sharing"/>
				<? } ?>
				<input type="hidden" name="idk" value="<?= $_REQUEST[idk]; ?>">
				<input type="hidden" name="sw" value="12">
			</p>
		</fieldset>
	</form>

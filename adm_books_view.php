...
if (!empty($_REQUEST['keyword']))
	$query .= " WHERE judul LIKE '%".$_REQUEST[keyword]."%' OR pengarang LIKE '%".$_REQUEST[keyword]."%' OR jenis LIKE '%".$_REQUEST[keyword]."%'";
...
# search table
?>
<table class="spacer" width="750">
<tr>
<td>
<form name="find" action="<?= $_SERVER["PHP_SELF"]?>?mn=5" method="post">
<input type="text" size="25" name="keyword" value="<?= $_REQUEST[keyword] ?>">
<input type="submit" name="submit" value="Cari" class="button">
</form></td><td align="right">	<form name="addkmap" action="<?= $_SERVER["PHP_SELF"]?>?mn=51" method="post">
	<input type="submit" value="Add a Book" class="button">
	</form>
</td>
</tr>
</table>

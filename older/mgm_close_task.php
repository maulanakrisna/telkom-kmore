...
$query = "SELECT a.*, b.nama, b.id_bidang, c.nm_loker FROM knowledge a JOIN user b ON a.nik=b.nik JOIN loker c ON b.id_bidang=c.id_loker WHERE sharing_status IN ('3','5') AND a.t_akhir < NOW()";
#echo "$query<br>";
if (!empty($_REQUEST[sortby]))
	$query .= " ORDER BY $_REQUEST[sortby] $_REQUEST[ascdesc]";
else
	$query .= " ORDER BY sharing_status DESC, t_mulai";
#echo "$query<br>";
...
<?
echo "</td><td align='right'><a href='?mn=33'>Closed Sharing Tasks</a></td></tr></table>";
echo "<center>";
...
?>
	<tr valign="top">
		...
		<td><? if ($row["sharing_status"]=='5') { $status="<font color='red'><b>Closing</b></font>"; ?>
			<a href="management.php?mn=31&idk=<?= $row["id_know"]; ?>" title="Close Sharing Approval"><?= $row["judul"]; ?></a>
			<? } else { $status="Open"; ?>
			<a href="management.php?mn=32&idk=<?= $row["id_know"]; ?>" title="Close Sharing Detail View"><?= $row["judul"]; ?></a>
			<? } ?></td>
		<td><?= $row["nama"]; ?></td>
		<td><?= $row["nm_loker"]; ?></td>
		<td><?= $status; ?></td>

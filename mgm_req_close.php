...
<?
echo "</td><td align='right'><a href='?mn=33'>Close Sharing Task History</a></td></tr></table>";
echo "<center>";
...
?>
	<tr valign="top">
		...
		<td><? if ($row["report_status"]=='1') { ?>
			<a href="management.php?mn=31&id=<?= $row["id_know"]; ?>" title="Close Sharing Approval"><?= $row["judul"]; ?></a>
			<? } else { ?>
			<a href="management.php?mn=32&id=<?= $row["id_know"]; ?>" title="View Close Sharing Detail"><?= $row["judul"]; ?></a>
			<? } ?></td>
		<td><?= $row["nama"]; ?></td>
		<td><?= $row["nm_loker"]; ?></td>
	</tr>
